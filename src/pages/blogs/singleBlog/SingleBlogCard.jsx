...
const customParsers = {
  ...
  // Add parsers for lists
  list: (block) => {
    const { style, items } = block.data
    const listType = style === "ordered" ? "ol" : "ul"

    // Handle items properly - they might be objects with content property
    const listItems = items
      .map((item) => {
        // If item is an object with content property
        if (typeof item === "object" && item !== null) {
          return `<li class="ml-4 mb-2 text-sm md:text-base">${item.content || JSON.stringify(item)}</li>`
        }
        // If item is a string
